...
function SingleMovieFetch(){
    ...
    useEffect(async() => {
        try {
            const response = await axios.get(`https://infinite-eyrie-76014.herokuapp.com/movie/${movid}`);
    ...
    return(
        <div>
            <Container fluid style={{padding:"5%", background:"#5fa796"}}>
                        ...
                        return (
                            <Col id={mov._id} key={mov._id} xs={6} md={4} lg={3} style={{marginBottom:"2%"}}>
                                <Card>
                                    ...
                                    <Card.Body style={{background:"#0aa3a1"}}>
